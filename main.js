...
if (navigator.geolocation) { 
  ...
  }).setText('George F. L. Charles Airport');
  ...
  map.on('click', 'facility_point', (e) => {
    ...
    // ポップアップの内容に画像を追加
    var popupContent = `<strong></strong> ${name} Bus Stop<br>
                        <img src="./tagedphoto/${name}.jpg" alt="${name}" style="width:200px;height:auto;">`;
...
map.on('load', () => {
  ...
  map.on('click', 'castries_bus', (e) => {
    if (e.features.length > 0) {
      ...
      // ポップアップの内容を設定
      const popupContent = `<strong>Route number:</strong> ${routeNumber}<br>
